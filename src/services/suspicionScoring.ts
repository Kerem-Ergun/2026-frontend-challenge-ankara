...
export interface Person {
    ...
    sightings: {
        asSubject: SightingRecord[];
        withOthers: SightingRecord[];
    };
...
export const getPodoLastKnownWhereabouts = (podoPerson: Person | null): { location: string; timestampMs: number } | null => {
    ...
    const movementEvents = [
        ...
        ...(podoPerson.sightings?.asSubject || []).map((sighting) => ({
            location: normalizeText((sighting as any).location),
...
export const calculateSuspicion = (
    person: Person,
    options: { podoLastKnownWhereabouts?: { location: string; timestampMs: number } | null; podoName?: string } = {}
): SuspicionResult => {
    ...
    const addFlag = (points: number, reason: string): void => {
        if (!points) return;
        score += points;
